...
t_bins_expt = min(spike_times_timeline):t_acg:max(spike_times_timeline);

if nargin == 1 || isempty(template_2)
    % Autocorrelogram

    % Find window with largest number of spikes
    binned_spikes_expt = histcounts(spike_times_timeline(spike_templates == template_1),t_bins_expt);
    [~,max_idx] = max(binned_spikes_expt);
    t_bins = t_bins_expt(max_idx):0.001:t_bins_expt(max_idx+1);
    
    % Bin spikes, autocorrelate, plot
    binned_spikes = histcounts(spike_times_timeline(spike_templates == template_1),t_bins);
    ...
    binned_spikes_1_expt = histcounts(spike_times_timeline(spike_templates == template_1),t_bins_expt);
    binned_spikes_2_expt = histcounts(spike_times_timeline(spike_templates == template_2),t_bins_expt);

    [~,max_idx] = max(normalize(binned_spikes_1_expt,'range').*normalize(binned_spikes_2_expt,'range'));
    t_bins = t_bins_expt(max_idx):0.001:t_bins_expt(max_idx+1);

    % Bin spikes, cross-correlate, plot
    binned_spikes_1 = histcounts(spike_times_timeline(spike_templates == template_1),t_bins);
    binned_spikes_2 = histcounts(spike_times_timeline(spike_templates == template_2),t_bins);
    [binned_spikes_xcorr,lags] = xcorr(binned_spikes_1,binned_spikes_2,max_lag);
    figure; plot(lags,smoothdata(binned_spikes_xcorr,'gaussian',t_smooth),'k');
    xline(0,'color','r');
    xlabel('Lag (ms)');
