function cellraster(align_times,align_groups)
% cellraster(align_times,align_groups)
%
...
plot_frac_max = 0.1; % fraction of max percentage to plot channels

max_amp = max(abs(gui_data.templates(gui_data.curr_unit,:,:)),[],2);
template_plot_channels = find(any(max_amp > max(max_amp*plot_frac_max,[],'all'),1));
...
switch eventdata.Key
    case 'downarrow'
        ...
        depth_sort = sortrows([template_tipdist(use_templates)',template_id(use_templates)'],1);
        new_unit = depth_sort(circshift(ismember(depth_sort(:,2), ...
            gui_data.curr_unit),-length(gui_data.curr_unit)),2);
