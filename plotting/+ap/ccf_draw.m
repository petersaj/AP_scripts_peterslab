    ...
    methods
        ...
        function obj = draw_probes_histology(obj,animal,probe_color)
            ...
            for curr_probe = 1:length(probe_ccf_vertices)
                ...
                probe_coords_mean = mean(probe_ccf_vertices{curr_probe},1);
                xyz = bsxfun(@minus,probe_ccf_vertices{curr_probe},probe_coords_mean);
                [~,~,V] = svd(xyz,0);
                ...
                [~,deepest_probe_idx] = max(probe_ccf_vertices{curr_probe}(:,2));
                probe_deepest_point = probe_ccf_vertices{curr_probe}(deepest_probe_idx,:);
