...
% (create pseudo-continuous flipper timestamps if clock resets between
% recordings, which happens if preview is stopped/started)
open_ephys_ttl_timestamps_backstep = find(diff(open_ephys_flipper.timestamps) < 0) + 1;
open_ephys_ttl_timestamps_continuous_offset = ...
    interp1([-1,0,open_ephys_ttl_timestamps_backstep], ...
    [-1,0,open_ephys_flipper.timestamps(open_ephys_ttl_timestamps_backstep-1)], ...
    1:length(open_ephys_flipper.timestamps),'previous','extrap')';

open_ephys_flipper.timestamps_continuous = open_ephys_flipper.timestamps + ...
    open_ephys_ttl_timestamps_continuous_offset;

% Resample Open Ephys flipper to DAQ sample rate
open_ephys_flipper_trace = logical(normalize(interp1( ...
    open_ephys_flipper.timestamps_continuous, ...
    single(open_ephys_flipper.value), ...
    open_ephys_flipper.timestamps_continuous(1):1/timelite.daq_info(1).rate: ...
    open_ephys_flipper.timestamps_continuous(end),'previous'),'range'));
...
curr_ephys_flipper_idx =  ...
    open_ephys_flipper.timestamps_continuous >= ephys_timelite_flipper_lag & ...
    open_ephys_flipper.timestamps_continuous <= ephys_timelite_flipper_lag + ...
...
% Waveform isn't as robust as it used to be? Maybe b/c Kilosort4

% (bombcell: I think I'd rather do this part myself)
...
% [ephysProperties, unitClassif] = bc.ep.runAllEphysProperties(char(kilosort_path), char(qMetrics_path), rerunEP, region);

% %%%% WORKING ON TAN DETECTION
% 
