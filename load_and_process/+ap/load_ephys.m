...
% Get recordings included in day
% (first recording after ephys starts until N recorded recordings)
day_recordings = plab.find_recordings(animal,rec_day).recording;
recording_datetimes = ...
...
ephys_included_recordings = find(ephys_datetime -  ...
    (recording_datetimes + minutes(1)) < 0, ...
    length(flipper_recording_idx)-1,'first');
ephys_recording_idx = strcmp(day_recordings(ephys_included_recordings),rec_time);

flipper_flip_times_ephys = open_ephys_flipper.timestamps( ...
    flipper_recording_idx(ephys_recording_idx):flipper_recording_idx(find(ephys_recording_idx)+1)-1);
