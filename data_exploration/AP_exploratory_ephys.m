...
%         curr_spikes = spike_times_timelite(use_spikes & (spike_templates == curr_unit));
% 
% %         % (surprisingly not faster??)
% %         curr_spikes_binned = reshape([histcounts(curr_spikes, ...
% %             reshape(t_peri_event',[],1)),NaN],size(t_peri_event'))';
% %         curr_mean_psth = mean(curr_spikes_binned(:,1:end-1),1);
% 
...
%         ap.print_progress_fraction(curr_unit,n_units);
%     end
% end
