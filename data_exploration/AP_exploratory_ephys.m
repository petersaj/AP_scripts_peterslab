...
if contains(bonsai_workflow,{'passive','Image'})
    ...
    quiescent_trials = arrayfun(@(x) ~any(wheel_move(...
        ...
        1:min(length(stimOn_times),length(align_category_all)));
...
set(gcf,'Name',sprintf('%s, %s, %s (%s)',animal,rec_day,rec_time,bonsai_workflow));
...
unit_handles = ap.plot_unit_depthrate([],false);
unit_handles.unit_dots.CData = +([1,0,0].*(event_response_p > 0.95)) + ([0,0,1].*(event_response_p < 0.05));
