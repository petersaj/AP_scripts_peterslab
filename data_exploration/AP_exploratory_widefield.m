...
if ~isfield(trial_events.values,'TaskType')
    ...
    trial_type = vertcat(trial_events.values.TaskType);
    stim_regressors = cell2mat(arrayfun(@(x) ...
        histcounts(stimOn_times(trial_type(1:length(stimOn_times))==x), ...
        time_bins),unique(trial_type),'uni',false));
end
...
%% Batch passive widefield kernel post-learning
...
task_recordings = plab.find_recordings(animal,[],use_workflow);

rxn_stat_p = nan(length(task_recordings),1);
...
for curr_recording = 1:length(task_recordings)
    ...
    clearvars('-except',preload_vars{:});
    AP_print_progress_fraction(curr_recording,length(task_recordings));
...
learned_days_idx = find(learned_day);
wf_k = cell(length(learned_days_idx),1);
for curr_recording = 1:length(learned_days_idx)
    ...
    rec_day = task_recordings(curr_recording).day;
    try
        rec_time = plab.find_recordings(animal,rec_day,'lcr_passive').recording{end};
    catch me
        continue
    end
    load_parts.widefield = true;
    ...
    wf_k{curr_recording} = kernels;

    % Clear vars except pre-load for next loop
    clearvars('-except',preload_vars{:});
    AP_print_progress_fraction(curr_recording,length(learned_days_idx));

end

wf_k_avg = nanmean(cat(4,wf_k{:}),4);
kernels_px = plab.wf.svd2px(wf_U(:,:,1:size(kernels,1)),wf_k_avg);
ap.imscroll(kernels_px,frame_shifts);
clim(max(abs(clim)).*[-1,1]);
colormap(ap.colormap('PWG'));
axis image
