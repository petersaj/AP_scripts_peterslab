...
for curr_animal = 1:length(animals)
    ...
    ap_thresh = 600;
    ap_annotation = cellfun(@(x) any(vertcat(x.ap) > ap_thresh), ...
        {AP_histology_processing.annotation.vertices_ccf});

    day_pattern = digitsPattern(4) + '-' + digitsPattern(2) + '-' + digitsPattern(2);
    rec_days = unique(extract({AP_histology_processing.annotation(ap_annotation).ephys_path},day_pattern));

    use_recs{curr_animal} = rec_days;
end
