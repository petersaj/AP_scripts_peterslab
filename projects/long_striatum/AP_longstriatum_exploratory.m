...
tan_psth_passive_cat = vertcat(tan_psth_all{use_animals});
tan_psth_passive_cat = vertcat(tan_psth_passive_cat{:});

load(fullfile(am_data_path,'tan_psth_all_task.mat'));

tan_psth_task_cat = vertcat(tan_psth_all{use_animals});
...
for curr_animal = find(use_animals)
...
% Make indicies per trial
day_animal = cell2mat(cellfun(@(x,y) repmat(x,length(y),1), ...
    num2cell(1:sum(use_animals)),{bhv(use_animals).learned_day},'uni',false)');
trial_animal = cell2mat(cellfun(@(x,y) repmat(x,size(y,1),1), ...
    num2cell(day_animal),vertcat(trial_data_all.widefield{use_animals}),'uni',false));

trial_ld = cell2mat(cellfun(@(x,y) repmat(x,size(y,1),1), ...
    num2cell(vertcat(ld{:})),vertcat(trial_data_all.widefield{use_animals}),'uni',false));

trial_idx = cell2mat(cellfun(@(x) repmat((1:size(x,1))',1), ...
    vertcat(trial_data_all.widefield{use_animals}),'uni',false));
...
% (passive)
if isfield(trial_data_all,'stim')
    trial_stim = cell2mat(vertcat(trial_data_all.stim{use_animals}));
end
if isfield(trial_data_all,'wheel_move')
    trial_wheel = cell2mat(vertcat(trial_data_all.wheel_move{use_animals}));
end
if isfield(trial_data_all,'wheel_velocity')
    trial_wheel_vel = cell2mat(vertcat(trial_data_all.wheel_velocity{use_animals}));
end
% (task)
if isfield(trial_data_all,'stim_to_move')
    trial_rxn = cell2mat(vertcat(trial_data_all.stim_to_move{use_animals}));
end
if isfield(trial_data_all,'outcome')
    trial_outcome = cell2mat(vertcat(trial_data_all.outcome{use_animals}));
end

% Widefield: concatenate trial data (and subtract baseline)
wf_cat = cell2mat(vertcat(trial_data_all.widefield{use_animals}));
wf_cat = wf_cat - nanmean(wf_cat(:,t<0,:),2);
...
% (need to get kidx elsewhere, choose kidx to use here)
use_str_depth = kidx == 3;
use_str_depth_day = mat2cell(use_str_depth,cellfun(@(x) size(x,3),vertcat(ctx_map_px{use_animals})),1);

softnorm = 20;
str_cat = cellfun(@(x,use_depth) (sum(x(:,:,use_depth),3) - nanmean(sum(x(:,t<0,use_depth),3),2)) ./ ...
    (nanmean(sum(x(:,t<0,use_depth),3),2) + softnorm), ...
    vertcat(trial_data_all.striatum_mua{use_animals}),use_str_depth_day,'uni',false, ...
