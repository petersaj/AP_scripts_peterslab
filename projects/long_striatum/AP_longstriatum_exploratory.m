...
for curr_animal = 1:length(animals)
    ...
    for curr_recording = 1:length(recordings)
        ...
        responsive_cells = event_response_p > 0.99;
...
for curr_kidx = 1:n_k
    ...
    str_mua_ldgrp_norm = ap.groupfun(@median,str_mua_depthgrp_norm,grp(:,2),[]);
    ...
    [m,e] = grpstats(str_mua_depthgrp_norm_tmax,grp(:,2),{'median','sem'});
...
r = mat2gray(ctx_map_cat.^2).*permute(x,[2,3,1]);
r2 = sqrt(sum(r.^2,3));
figure;imagesc(r2); 
