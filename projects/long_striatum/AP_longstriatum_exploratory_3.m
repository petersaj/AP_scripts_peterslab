...
for curr_day_grp = 1:length(plot_day_bins)-1
    ...
    scatter(unit_psth_max(curr_units,2),unit_psth_max(curr_units,3), ...
        10+20*(striatum_units_responsive_category(curr_units)~=1), ...
...
% Average response to stimuli by responsiveness
stim_colormap = ap.colormap('BKR',n_stim);
for curr_stim = 2:3
    for curr_day_grp = 1:length(plot_day_bins)-1
...
linkaxes(h.Children(length(h.Children):-1:1>length(plot_day_bins)-1),'xy');
linkaxes(h.Children(~(length(h.Children):-1:1>length(plot_day_bins)-1)),'xy');
...
figure;bar(["C","R","C+R"],frac_responsive);
