...
% % (TESTING: normalize to passive LD0)
% str_normval = data_grids.striatum_passive(:,find(data_grid_params.ld_bins>=0,1),:,3);
% wf_normval = data_grids.wf_roi_passive(:,find(data_grid_params.ld_bins>=0,1),:,3);
% wf_kernel_normval = data_grids.wf_kernel_roi_passive(:,find(data_grid_params.ld_bins>=0,1),:,3);
...
axis square;
xlabel('Passive');ylabel('Task');
legend(gca().Children(end:-2:1),vertcat(legend_labels,'Identity ref'), ...
    'location','best')
ap.prettyfig;

% Plot cortex vs striatum for passive/task overlaid
figure;
h = tiledlayout(1,2);
for curr_domain = 1:2
    ...
    curr_cortex_passive = reshape(data_grids.wf_roi_passive(:,:,curr_domain,3),[],1);
    curr_cortex_task = reshape(data_grids.wf_roi_task(:,:,curr_domain),[],1);

    curr_striatum_passive = reshape(data_grids.striatum_passive(:,:,curr_domain,3),[],1);
    curr_striatum_task = reshape(data_grids.striatum_task(:,:,curr_domain),[],1);

    % Max-normalize
    curr_cortex_act = [curr_cortex_passive,curr_cortex_task];
    curr_striatum_act = [curr_striatum_passive,curr_striatum_task];

    % Scatter
    h_dot = scatter(curr_striatum_act(:,1),curr_cortex_act(:,1),20,'filled');
    h_dot = scatter(curr_striatum_act(:,2),curr_cortex_act(:,2),20,'filled');
...
% (wf dff, norm, errorbar)
plot_task = @(roi,str,col) ...
    errorbar(nanmean(data_grids.striatum_task(:,plot_ld_idx,str)./str_normval(:,:,str),1), ...
    nanmean(data_grids.wf_roi_task(:,plot_ld_idx,roi)./wf_normval(:,:,roi),1), ...
    ap.sem(data_grids.wf_roi_task(:,plot_ld_idx,roi)./wf_normval(:,:,roi),1), ...
    ap.sem(data_grids.wf_roi_task(:,plot_ld_idx,roi)./wf_normval(:,:,roi),1), ...
    ap.sem(data_grids.striatum_task(:,plot_ld_idx,str)./str_normval(:,:,str),1), ...
    ap.sem(data_grids.striatum_task(:,plot_ld_idx,str)./str_normval(:,:,str),1), ...
    '.-','color',col,'linewidth',2,'MarkerSize',30,'capsize',0);
plot_passive = @(roi,str,plot_stim,col) ...
    errorbar(nanmean(data_grids.striatum_passive(:,plot_ld_idx,str,plot_stim)./str_normval(:,:,str),1), ...
    nanmean(data_grids.wf_roi_passive(:,plot_ld_idx,roi,plot_stim)./wf_normval(:,:,roi),1), ...
    ap.sem(data_grids.wf_roi_passive(:,plot_ld_idx,roi,plot_stim)./wf_normval(:,:,roi),1), ...
    ap.sem(data_grids.wf_roi_passive(:,plot_ld_idx,roi,plot_stim)./wf_normval(:,:,roi),1), ...
    ap.sem(data_grids.striatum_passive(:,plot_ld_idx,str,plot_stim)./str_normval(:,:,str),1), ...
    ap.sem(data_grids.striatum_passive(:,plot_ld_idx,str,plot_stim)./str_normval(:,:,str),1), ...
    '.-','color',col,'linewidth',2,'MarkerSize',30,'capsize',0);
...
axis equal square
% line(xlim,xlim,'color',[0.5,0.5,0.5]);
ylabel(sprintf('Cortex (LD0-norm)'));
xlabel('Striatum (LD0-norm)')
ap.prettyfig;
title('Task');

nexttile;hold on;
h1 = plot_passive(2,1,3,'r');
h2 = plot_passive(2,2,3,'k');
outline_ld(h1); outline_ld(h2);
axis equal square
% line(xlim,xlim,'color',[0.5,0.5,0.5]);
ylabel(sprintf('Cortex (LD0-norm)'));
...
% passive str1/2 vs mpfc
figure;tiledlayout(1,2);
nexttile; hold on;
h1 = plot_passive(2,1,1,'b');
h2 = plot_passive(2,1,2,'k');
h3 = plot_passive(2,1,3,'r');
outline_ld(h1);outline_ld(h2);outline_ld(h3);

nexttile; hold on;
h1 = plot_passive(2,2,1,'b');
h2 = plot_passive(2,2,2,'k');
h3 = plot_passive(2,2,3,'r');
outline_ld(h1);outline_ld(h2);outline_ld(h3);
ap.prettyfig; 

% Task + passive str1/2 vs mpfc
figure;tiledlayout(1,2);
nexttile; hold on;
h1 = plot_task(2,1,'r');
h2 = plot_passive(2,1,3,'k');
outline_ld(h1);outline_ld(h2);
title('Str 1');
nexttile; hold on;
h1 = plot_task(2,2,'r');
...
% 3D passive str1,2,mpfc
figure; hold on;
stim_col = {'b','k','r'};
for curr_stim = 1:3
    plot3(nanmean(data_grids.striatum_passive(:,plot_ld_idx,1,curr_stim)./str_normval(:,:,1),1), ...
        nanmean(data_grids.striatum_passive(:,plot_ld_idx,2,curr_stim)./str_normval(:,:,2),1), ...
        nanmean(data_grids.wf_roi_passive(:,plot_ld_idx,2,curr_stim)./wf_normval(:,:,2),1), ...
        '.-','color',stim_col{curr_stim},'linewidth',2,'MarkerSize',30);
end
xlabel('Str1');ylabel('Str2');zlabel('mPFC');
view([45,45]);axis vis3d;


% normalizing differently
% (div separately in task/passive)
...
% ctx_task_norm = (data_grids.wf_roi_task-data_grids.wf_roi_task(:,1,:))./data_grids.wf_roi_task(:,3,:);
% 
% use_stim = 3;
% str_passive_norm = (data_grids.striatum_passive(:,:,:,use_stim)-data_grids.striatum_passive(:,1,:,use_stim))./data_grids.striatum_task(:,3,:);
% ctx_passive_norm = (data_grids.wf_roi_passive(:,:,:,use_stim)-data_grids.wf_roi_passive(:,1,:,use_stim))./data_grids.wf_roi_task(:,3,:);

% % (sub = min)
% str_task_norm = (data_grids.striatum_task-min(data_grids.striatum_task,[],2))./data_grids.striatum_task(:,3,:);
% ctx_task_norm = (data_grids.wf_roi_task-min(data_grids.wf_roi_task,[],2))./data_grids.wf_roi_task(:,3,:);
% 
% use_stim = 3;
% str_passive_norm = (data_grids.striatum_passive(:,:,:,use_stim)-min(data_grids.striatum_passive(:,:,:,use_stim),[],2))./data_grids.striatum_task(:,3,:);
% ctx_passive_norm = (data_grids.wf_roi_passive(:,:,:,use_stim)-min(data_grids.wf_roi_passive(:,:,:,use_stim),[],2))./data_grids.wf_roi_task(:,3,:);

figure; tiledlayout(1,2);
nexttile; hold on;
plot(nanmean(str_task_norm(:,:,1),1),nanmean(ctx_task_norm(:,:,2),1),'.-','linewidth',2,'MarkerSize',30)
plot(nanmean(str_task_norm(:,:,2),1),nanmean(ctx_task_norm(:,:,2),1),'.-','linewidth',2,'MarkerSize',30)
title('Task');
line(ylim,ylim)

nexttile; hold on;
plot(nanmean(str_passive_norm(:,:,1),1),nanmean(ctx_passive_norm(:,:,2),1),'.-','linewidth',2,'MarkerSize',30)
plot(nanmean(str_passive_norm(:,:,2),1),nanmean(ctx_passive_norm(:,:,2),1),'.-','linewidth',2,'MarkerSize',30)
title('Passive');
line(ylim,ylim)
...
% difference from unity ctx=str
% (norm = div)
str_task_norm = data_grids.striatum_task./str_normval;
ctx_task_norm = data_grids.wf_roi_task./wf_normval;

str_passive_norm = data_grids.striatum_passive(:,:,:,3)./str_normval;
ctx_passive_norm = data_grids.wf_roi_passive(:,:,:,3)./wf_normval;

% % (norm = bin1 sub and div)
% str_task_norm = (data_grids.striatum_task-data_grids.striatum_task(:,1,:))./data_grids.striatum_task(:,3,:);
% ctx_task_norm = (data_grids.wf_roi_task-data_grids.wf_roi_task(:,1,:))./data_grids.wf_roi_task(:,3,:);
% 
% use_stim = 3;
...
% ctx-str difference from bin 1

use_str = 1;
use_ctx = 2;
str_ctx_diff = str_task_norm(:,:,use_str)-ctx_task_norm(:,:,use_ctx);

stat_meas = nanmean(diff(str_ctx_diff(:,1:2),[],2),1);

n_shuff = 10000;
stat_shuff = nan(n_shuff,size(stat_meas,2));
for curr_shuff = 1:n_shuff
    curr_shuff_data = ap.shake(str_ctx_diff(:,1:2),2);
    stat_shuff(curr_shuff,:) = nanmean(diff(curr_shuff_data,[],2),1);
end
stat_rank = tiedrank([stat_meas;stat_shuff]);
stat_p = 1-stat_rank(1,:)/(n_shuff+1)
...
use_str = 1;
use_ctx = 2;
str_ctx_diff = str_passive_norm(:,:,use_str)-ctx_passive_norm(:,:,use_ctx);
...
% cortex vs striatum difference from bin 1

use_str = 1;
use_ctx = 2;

stat_meas = nanmean((str_task_norm(:,2,use_str) - str_task_norm(:,1,use_str)) - ...
    (ctx_task_norm(:,2,use_ctx) - ctx_task_norm(:,1,use_ctx)),1);
...
for curr_shuff = 1:n_shuff
    ...
    stat_shuff(curr_shuff,:) = nanmean(diff(ap.shake(str_task_norm(:,1:2,use_str),2),[],2) - ...
        diff(ap.shake(ctx_task_norm(:,1:2,use_ctx),2),[],2));
end
stat_rank = tiedrank([stat_meas;stat_shuff]);
stat_p = 1-stat_rank(1,:)/(n_shuff+1)


stat_meas = nanmean((str_passive_norm(:,2,use_str) - str_passive_norm(:,1,use_str)) - ...
    (ctx_passive_norm(:,2,use_ctx) - ctx_passive_norm(:,1,use_ctx)),1);
...
for curr_shuff = 1:n_shuff
    ...
    stat_shuff(curr_shuff,:) = nanmean(diff(ap.shake(str_passive_norm(:,1:2,use_str),2),[],2) - ...
        diff(ap.shake(ctx_passive_norm(:,1:2,use_ctx),2),[],2));
end
stat_rank = tiedrank([stat_meas;stat_shuff]);
stat_p = 1-stat_rank(1,:)/(n_shuff+1)
...
plot_lds = -3:2;
figure; h = tiledlayout(n_domains,length(plot_lds));
for curr_domain = 1:n_domains
    for curr_ld = plot_lds
...
linkaxes(h.Children,'xy');
legend({'Stim move-nan','Stim-move stim-nan','No-stim move'})


% Striatum day bins
% (not saved by trial - so by weighted sum of no-stim move)
plot_day_bins = [-Inf,0,Inf];
...
%% --> Non-stim move activity (subset of above) 
