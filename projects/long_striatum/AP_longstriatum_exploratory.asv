...
for curr_animal = 1:length(animals)
    ...
    for curr_recording = 1:length(recordings)
        ...
        use_tans = find(tans & template_depths >= striatum_depth(1) & template_depths <= striatum_depth(2));
...
for curr_kidx = 1:n_k
    ...
    [m,e] = grpstats(stim_cells_frac,grp(:,2),{'median','sem'});
...
% (have to truncate bc more tan depths than ctx sometimes??)
tan_psth_cat = cellfun(@(x,y) x(1:size(y,2)), ...
    vertcat(tan_psth_all{:}), vertcat(ctx_map_all{:}),'uni',false);
...
for i = 1:numel(tan_grp)
    ...
    curr_tans = tan_grp{i}(:,:,3);
    [~,sort_idx] = sort(mean(curr_tans(:,500:700),2));
    imagesc(curr_tans(sort_idx,:));
    clim([-2,2]);
    title(sprintf('LD %d',ld_unique(i)));
end
...
use_trials = trial_ld == -1 & trial_rxn > 0.2;
avg_px = plab.wf.svd2px(U_master(:,:,1:n_vs), ...
...
% (split within day)
% use_trials = find(trial_stim == 90 &  ~any(trial_wheel(:,t>0 & t<=0.3),2) & trial_ld == 0);
use_trials = find(trial_ld < 0);

n_split = 4;
use_trial_split = discretize(tiedrank( ...
    nanmean(roi_trace(use_trials,t > 0 & t < 0.2),2))./ ...
    length(use_trials),linspace(0.2,0.8,n_split+1));

figure; hold on; set(gca,'colororder',ap.colormap('KR',n_split));
str_grp_mean = ap.groupfun(@nanmean,str_cat(use_trials(~isnan(use_trial_split)),:), ...
    use_trial_split(~isnan(use_trial_split)),[]);
plot(str_grp_mean','linewidth',2);

curr_wf_split = ap.groupfun(@nanmean,wf_cat(use_trials(~isnan(use_trial_split)),:,:), ...
    use_trial_split(~isnan(use_trial_split)),[],[]);
...
% (split across all data)

use_trials_all = trial_stim == 90 &  ~any(trial_wheel(:,t>0 & t<=0.3),2);
% use_trials_all = true(size(trial_ld));

n_split = 5;
trial_split = nan(size(trial_ld));

trial_split(use_trials_all) = discretize(tiedrank( ...
    nanmean(roi_trace(use_trials_all,t > 0.05 & t < 0.15),2))./ ...
    sum(use_trials_all),linspace(0.2,0.8,n_split+1));

% trial_split(use_trials_all) = discretize(tiedrank( ...
%     nanmean(str_cat(use_trials_all,t > 0.05 & t < 0.15),2))./ ...
%     sum(use_trials_all),linspace(0.2,0.8,n_split+1));
