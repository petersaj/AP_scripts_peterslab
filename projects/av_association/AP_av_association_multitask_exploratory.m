...
%% Task-specific: KERNEL passive stim (testing logistic)
...
% [B,FitInfo] = lassoglm(wf_V(1:n_components,:)',stim_regressors(2,:)','binomial','NumLambda',25,'CV',5);
...
% GLM - not regularized
model = fitglm(regressors_flat, stim_regressors(3,:)','linear','Distribution','binomial','Link','logit');
...
% Model - elastic net (can't get pure ridge alpha=0)
[B, FitInfo] = lassoglm(regressors_flat, stim_regressors(3,:)','binomial','Alpha',1e-5,'Lambda',1e-2);
...
[B, FitInfo] = lassoglm(regressors_flat(use_t,:), stim_regressors(3,use_t)','binomial','Alpha',1e-7,'Lambda',1e-1,'CV',5);
...
model = fitclinear(regressors_flat, stim_regressors(3,:)', 'Learner', 'logistic', 'Regularization', 'ridge', 'Lambda', 0);
